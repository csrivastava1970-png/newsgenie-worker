...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === "/api/digi-pack" && request.method === "POST") {
      ...
      if (!has_openai_key) {
        return json(
          { ok: false, path, entry_marker: ENTRY_MARKER, has_openai_key, error: "OPENAI_API_KEY missing" },
          500,
          corsHeaders(request)
        );
      }

      // Structured output schema (bullet_points / vo_lines / hashtags)
                  // Structured output schema (DigiPack V2: WEB + VIDEO + SOCIAL)
      const schema = {
        type: "object",
