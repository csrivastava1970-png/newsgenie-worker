...
function normalizeRowLayout(row) {
  ...
  // Find remove button robustly (class OR danger OR text)
  var rb =
    row.querySelector("button.ng-byte-remove") ||
    row.querySelector("button.ng-remove-byte") ||
  ...
  if (!rb) {
    ...
    for (var i = 0; i < btns.length; i++) {
      ...
      if (t === "remove" || t.indexOf("remove") >= 0) { rb = btns[i]; break; }
    }
  }
  if (!rb) return false;
...
window.__NG_BYTES_NORM__ = {
  ...
  // NOTE: row-normalize is internal; we expose all() only to keep it simple
};

function installNormalizeObserver() {
  if (window.__NG_BYTES_NORM_OBS__) return;

  var wrap = document.getElementById("bytesWrap") || document.querySelector('[data-ng-sec="bytes"]');
  if (!wrap) return;
  ...
  var rows = document.querySelectorAll("[data-byte-row], .ng-byte-row, .row");
