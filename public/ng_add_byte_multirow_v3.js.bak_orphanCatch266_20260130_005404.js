...
function normalizeRowLayout(row) {
  ...
  // Try DOM move: place remove immediately after textarea
  try {
...
function getBytesRoot() {
  ...
  // Fallback: find any Remove control anywhere, then climb to a panel-like root
  var nodes = Array.from(document.querySelectorAll("button, input[type=button], input[type=submit], [role=button]"));
  var any = nodes.find(function (el) {
  ...
  if (any.closest) {
    return any.closest("#bytesWrapInner")
      || any.closest("#bytesWrap")
      || any.closest('[data-ng-sec="bytes"]')
      || any.closest(".panel")
      || null;
  }
...
window.__NG_BYTES_NORM__ = { all: normalizeAllRows };
